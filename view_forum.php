...
<?php
if(isset($_GET['id'])){
$qry = $conn->query("SELECT t.*,u.firstname FROM topics t inner join users u on u.id = t.user_id where t.id= ".$_GET['id']);
foreach($qry->fetch_array() as $k => $val){
	$$k=$val;
}
$comments = $conn->query("SELECT c.*,u.firstname,u.username FROM comments c inner join users u on u.id = c.user_id where c.topic_id= ".$_GET['id']." order by unix_timestamp(c.date_created) asc");
$com_arr= array();
while($row= $comments->fetch_assoc()){
	$com_arr[] = $row;
}
$replies = $conn->query("SELECT r.*,u.firstname,u.username FROM replies r inner join users u on u.id = r.user_id where r.comment_id in (SELECT id FROM comments where topic_id= ".$_GET['id'].") order by unix_timestamp(r.date_created) asc");
$rep_arr= array();
		...
		<div class="card">
    		...
    		<div class="col-lg-12">
    			...
    			<div class="form-group comment">
    				...
    				<p class="mb-0"><large><b><?php echo $row['firstname'] ?></b></large>  <span class="text-primary"><small class="mb-0"><i><?php echo $row['username'] ?></i></small></span></p>
    				...
    				<div>
    					...
    					<div class="col-lg-8 offset-lg-2 replies">
    						...
    						<div class="form-group ty-compact-list">
			    				...
			    				<p class="mb-0"><large><b><?php echo $rep['firstname'] ?></b></large>  <span class="text-primary"><small class="mb-0"><i><?php echo $rep['username'] ?></i></small></span></p>
